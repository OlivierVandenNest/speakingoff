...
import micLogo from "../mic_logo.png";
    ...
    return (
        <div className="mt-4 p-3 block-example border border-success rounded mb-0" onClick={() => setClicked(!clicked)}>
            <div className="meeting-detail-top">
                ...
                {clicked && (
                    <Button className="meeting-detail-rightgroup mr-2" variant="success">
                        Add Tag
                    </Button>
                )}
                {clicked && <Button className="mr-2" variant="outline-success">Remove Tag</Button>}
                <Button className={clicked ? "" : "meeting-detail-rightgroup"} variant={clicked ? "danger" : "success"}>{clicked ? "Hide" : "Show"}</Button>
            </div>
            {clicked && <div className="mt-3 meeting-detail-mid">
                <ProgressBar className="meeting-detail-progressbar" animated now={progress} variant="success" />
            ...
            {clicked && <div className="meeting-detail-bottom">
